...
| 数据 | 去向 | 说明 |
|---|---|---|
| 标签页音频 | 扩展 → `127.0.0.1:8787`（本机后端），或你在 popup 指定的**局域网后端** | 16 kHz PCM，只在内存中流转；**不录制、不落盘**。指向局域网后端时音频以明文 WebSocket 经过局域网，且后端无鉴权，只应在可信网络使用 |
...
| `optional_host_permissions: http://*/*, ws://*/*` | 仅当你在 popup 填入非本机后端地址并点「应用」时，才为**那一个地址**申请权限 |
