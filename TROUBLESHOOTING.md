...
| popup「后端未响应（http://192.168.x.x:8787/healthz）」 | 远程后端只监听 `127.0.0.1`、没启动、或防火墙拦截；或扩展没有该地址的权限 | 那台机器用 `HOST=0.0.0.0 npm run start:server`（或写进它的 `.env`），`curl http://192.168.x.x:8787/healthz` 从本机能通；popup「诊断 → 后端地址」重新点「应用」授予权限。不要在源码里改 `BACKEND_WS_URL`，用 popup 设置 |
